...
def process_terms(in_path, k1=1.5, b=0.75):
    ...
    # Save to pickle
    with (
        open(Path("data/abs_bm25.pickle"), "wb") as f1,
        open(Path("data/term_idf_bm25.pickle"), "wb") as f2,
    ):
        pickle.dump(bm25_vectors, f1)
        pickle.dump(term_idf, f2)

    # Also save avg_doc_len and doc_lengths for query scoring
    with open(Path("data/bm25_params.pickle"), "wb") as f3:
...
def bm25_search(queries, output_file, k1=1.5, b=0.75):
    ...
    """
    abstract_vectors = load_pickle(Path("data/abs_bm25.pickle"))
    term_idf = load_pickle(Path("data/term_idf_bm25.pickle"))
    params = load_pickle(Path("data/bm25_params.pickle"))
